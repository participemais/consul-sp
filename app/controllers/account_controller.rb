  ...
  private
    ...
    def set_geozone
      ...
      if @account.address_changeable? && account_params[:district_id].blank? && @account.level_three_verified? && @account.from_sp? && ( @account.cep_changed? || @account.geozone == nil )
        ...
        flash.now[:alert] = "Não é permitido atualizar dados de endereço dentro de um intervalo de 30 dias da última alteração! Medida aplicada devido possibilidade de ocorrência de votações restritas por localidade (distrito ou subprefeitura)."
        return render :show
      end
    end
